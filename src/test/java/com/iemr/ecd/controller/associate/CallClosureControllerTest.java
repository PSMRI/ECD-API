...
class CallClosureControllerTest {

	@Mock
	private CallClosureImpl callClosureImpl;

	@InjectMocks
	private CallClosureController controller;

	@Test
	void closeCallDelegatesToService() {
		CallClosureDTO request = new CallClosureDTO();
		when(callClosureImpl.closeCall(request)).thenReturn("closed");

		ResponseEntity<String> response = controller.closeCall(request);
